...
支持MetaTrader 5外汇黄金EA和币圈交易所交易的量化交易平台，集成多模型决策引擎、智能持仓监控、动态止损、数据源集成、策略训练、盈亏分析、风险控制、自动复盘、回测仿真、实盘数据采集、性能监控、自动更新、CPU限制器、双服务器部署、高可用架构等功能。
...
│   └── trailing_stop_engine.py    # 动态止损引擎
